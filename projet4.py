...
# projet4.py : Annuaire de super-Heros
# Servais Nikita
        ...
        with open(fichier, encoding = 'utf8') as f :
            ...
            for line in  f :
                ...
                if hero[1] in annuaire :
                    ...
                    annuaire[hero[1]] = {hero[0] : hero_info}
            if not test_file :
                print("Vous n’avez pas de super-héro "\
                      "actuellement dans l’annuaire,"\
                      " veuillez effectuer")
...
def add_hero(annuaire) :
    ...
    while (len(tel) != 14 or ((tel[0]+tel[4]+tel[5]+tel[9] != '()--')
            ...
            print("Votre numéro de téléphone doit "\
                  "être composé de 3 entiers séparés par ’-’")
            print("dans le format (CCC)-CCC-CCCC.")
            print("Veuillez réessayer d’entrer le numéro.")
            print("Quel est son numéro de téléphone ?")
            tel = str(input())
    hero_info_list.append(('telephone',tel))
    print("Quels sont ses super-pouvoirs ? "\
          "(Entrez 0 pour arrêter l’enregistrement)")
    superpouvoir = []
    pouvoir = ""
    while pouvoir != "0" :
        if pouvoir.strip() :
...
def show_hero(annuaire) :
    ...
    while not sortir :
        dic={}
        print("Veuillez choisir une ville pour y voir les heros disponibleS")
        ...
        while not test :
            try :
                ...
                test = False
        dic = {}
        print("Veuillez choisir un hero "\
              "pour y voir les informations disponibles")
        ...
        while not test :
            try :
                ...
                print("Vous devez choisir entre "+
                      ...
                      " ou "+str(len(dic))+" !")
                test = False
        print(choix_hero+" possède une puissance de "+
              str(annuaire[choix_ville][choix_hero]["puissance"]))
        print("    son/ses super-pouvoir :")
...
def menu(fichier) :
    """
    Permet d'acceder au differente fonctionalité du programme
    """
    sortir = False
